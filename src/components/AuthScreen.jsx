...
export default function AuthScreen({ onLoginSuccess }) {
  const { loginWithEmail, signupWithEmail, loginWithGoogle, isConfigured, sendPasswordReset } = useAuth();
  ...
  // Automatically clear errors, successes, and loading state on authMode change
  useEffect(() => {
    setErrorMessage("");
    setSuccessMessage("");
    setLoading(false);
  ...
  // Handle auto-dismissing error message after 5 seconds
  useEffect(() => {
    if (errorMessage) {
  ...
  // Handle auto-dismissing success message after 5 seconds
  useEffect(() => {
    if (successMessage) {
  ...
  const handleForgotPasswordSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMessage("If an account exists for this email, we've sent a password reset link. Please check your inbox and spam folder.");
      setEmail("");
    } catch (err) {
          ...
          <div className="auth-main-row">
            ...
            <div className="auth-form-viewport">
              ...
              <div className="auth-top-nav">
                <span className="landing-back-link" onClick={() => { setScreenMode("landing"); }}>
                  ← Back
                </span>
                <span className="auth-nav-toggle">
                  {authMode === "login" ? (
                    <>New to PrepSim? <span className="auth-nav-link" onClick={() => { setAuthMode("signup"); }}>Sign up</span></>
                  ) : authMode === "signup" ? (
                    <>Have an account? <span className="auth-nav-link" onClick={() => { setAuthMode("login"); }}>Sign in</span></>
                  ) : (
                    <span className="auth-nav-link" onClick={() => { setAuthMode("login"); }}>Back to Sign in</span>
                  )}
                </span>
              </div>
              ...
              <div className="auth-card-custom">
                ...
                <p className="card-subtitle-custom">
                  {authMode === "login" && "Sign in to continue to PrepSim"}
                  {authMode === "signup" && "Register to continue to PrepSim"}
                  {authMode === "forgot" && "Enter the email associated with your account and we'll send you a secure password reset link."}
                </p>
                ...
                {authMode === "forgot" ? (
                  <form onSubmit={handleForgotPasswordSubmit}>
                    ...
                    <div style={{ marginTop: "20px", fontSize: "0.9rem" }}>
                      <span className="auth-nav-link" onClick={() => setAuthMode("login")} style={{ marginLeft: 0 }}>
                        Back to Sign In
                      </span>
                    </div>
                  </form>
                  ...
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="remember-row-custom">
                      ...
                      <a href="#forgot" className="forgot-link-custom" onClick={(e) => { e.preventDefault(); setAuthMode("forgot"); }}>
                        Forgot password?
                      </a>
                    </div>
